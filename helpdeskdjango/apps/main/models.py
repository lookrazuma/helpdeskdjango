...
class status(models.Model):
    """ 
    Модель статуса. Не используется. Лень убрать
    """
...
class Task(models.Model):
    ...
    # Поле для диаграммы, нигде не отображается
    ...
    comment = models.TextField('Коментарий от исполнителя', default='', blank=True, null=True)
    comment_author = models.TextField("Отзыв заявителя", default='', blank=True, null=True)
...
class Profile(models.Model):
    ...
    photo = models.ImageField(upload_to='user-profile-photo/', blank=True, null=True, default='')
