...
class TaskView(StatusTask, ListView):
    paginate_by = 5 # Количество заявок на одной странице
    model = Task
...
class SearchResultsView(ListView):
    ...
    def get_queryset(self): # новый
        ...
        object_list = Task.objects.filter(
            ...
            | Q(status_task__icontains=query), author=username 
        )
        return object_list
...
class ProfileDetailView(DetailView):
    ...
    def get_queryset(self):
        photo = self.model.objects.get(username=self.request.user)
        print(photo)
        return render({
        'photo' : photo
    }) 
...
class ProfileUpdateView( LoginRequiredMixin,View, ObjUpdateMixin):
    ...
    def get(self, request):
        datas = self.data.objects.get(request.FILES, username=self.request.user.id)
    ...
    if request.user.email:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                mail_client.send()
                if mail_client and request.user.email != '':
